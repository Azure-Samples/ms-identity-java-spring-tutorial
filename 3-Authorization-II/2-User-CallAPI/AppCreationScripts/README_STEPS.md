...
1. This Web application will leverage the [Microsoft Authentication Library \(MSAL\) for Java](https://github.com/AzureAD/microsoft-authentication-library-for-java) SDK to sign-in a user and obtain an [Access Token](https://aka.ms/access-tokens) for a protected Web Api.
1. The Web app then calls the Java Spring Web API that is secured using Azure AD. The Api is secured using the [Azure AD Spring Boot Starter client library for Java](https://github.com/Azure/azure-sdk-for-java/tree/master/sdk/spring/azure-spring-boot-starter-active-directory). 
