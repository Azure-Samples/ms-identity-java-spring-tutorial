...
Function Cleanup
{
    ...
    Write-Host "Removing 'client' (java-spring-daemon-auth) if needed"
    try
    {
        Get-MgApplication -Filter "DisplayName eq 'java-spring-daemon-auth'" | ForEach-Object {Remove-MgApplication -ApplicationId $_.Id }
    }
    catch
    {
        $message = $_
        Write-Warning $Error[0]
        Write-Host "Unable to remove the application 'java-spring-daemon-auth'. Error is $message. Try deleting manually." -ForegroundColor White -BackgroundColor Red
    }

    Write-Host "Making sure there are no more (java-spring-daemon-auth) applications found, will remove if needed..."
    $apps = Get-MgApplication -Filter "DisplayName eq 'java-spring-daemon-auth'" | Format-List Id, DisplayName, AppId, SignInAudience, PublisherDomain
    ...
    {
        Remove-MgApplication -ApplicationId $app.Id
        Write-Host "Removed java-spring-daemon-auth.."
    }

    # also remove service principals of this app
    try
    {
        Get-MgServicePrincipal -filter "DisplayName eq 'java-spring-daemon-auth'" | ForEach-Object {Remove-MgServicePrincipal -ServicePrincipalId $_.Id -Confirm:$false}
    }
    catch
    {
        $message = $_
        Write-Warning $Error[0]
        Write-Host "Unable to remove ServicePrincipal 'java-spring-daemon-auth'. Error is $message. Try deleting manually from Enterprise applications." -ForegroundColor White -BackgroundColor Red
    }
}
