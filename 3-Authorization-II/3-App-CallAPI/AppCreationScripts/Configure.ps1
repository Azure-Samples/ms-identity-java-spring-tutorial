...
Function ConfigureApplications
{
   ...
   Write-Host "Creating the AAD application (java-spring-daemon-auth)"
   # Get a 6 months application key for the client Application
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInMonths 6
   
   # create the application 
   $clientAadApplication = New-MgApplication -DisplayName "java-spring-daemon-auth" `
                                                      -Web `
                                                      @{ `
                                                        } `
                                                       -SignInAudience AzureADMyOrg `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/java-spring-daemon-auth")
    ...
    Write-Host "Done creating the client application (java-spring-daemon-auth)"
    ...
    Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>java-spring-daemon-auth</a></td></tr>" -Path createdApps.html
    ...
    # print the registered app portal URL for any further navigation
    Write-Host "Successfully registered and configured that app registration for 'java-spring-daemon-auth' at `n $clientPortalUrl" -ForegroundColor Green 
    ...
    # $configFile = $pwd.Path + "\..\daemon\src\main\resources\application.yml"
    $configFile = $(Resolve-Path ($pwd.Path + "\..\daemon\src\main\resources\application.yml"))
