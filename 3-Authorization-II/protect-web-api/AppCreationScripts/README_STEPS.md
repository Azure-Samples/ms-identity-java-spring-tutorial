...
name: Enable your Java Spring Boot web app to sign in users on your Azure Active Directory and call a protected Web api
...
# Enable your Java Spring Boot web app to sign in users on your Azure Active Directory and call a protected Web api
...
1. Select **Add a scope** button open the **Add a scope** screen and Enter the values as indicated below:
    ...
    > Repeat the steps above for another scope named **ReadWrite**
...
1. Since this app signs-in users, we will now proceed to select **delegated permissions**, which is is required by apps signing-in users.
   ...
   1. In the list of APIs, select the API `java-spring-webapi-auth`.
      * Since this app signs-in users, we will now proceed to select **delegated permissions**, which is is requested by apps when signing-in users.
           1. In the **Delegated permissions** section, select the **Read**, **ReadWrite** in the list. Use the search box if necessary.
...
1. In the root directory as this project, run `cd java-spring-webapi-auth`. 
1. run `mvn clean compile spring-boot:run`.
1. Open a terminal or the integrated VSCode terminal.
1. In the root directory as this project, run `cd java-spring-webapp-auth`. 
1. run `mvn clean compile spring-boot:run`.
...
#### Step 2: Publish your files (java-spring-webapi-auth)
...
1. A prompt should appear after a few minutes stating that deployment is complete1.
...
> :note: The maven plugin uses information from your subscription to deploy to Azure App Services. Deployment is subject to the limtations of your subscription. Modify your pom.xml to meet your subscription limits.
...
1. In the app's registration screen, select **Authentication** in the menu.
   - In the **Redirect URIs** section, update the reply URLs to match the site URL of your Azure deployment. For example:
      - `https://java-spring-webapp-auth.azurewebsites.net/login/oauth2/code/`
